...
# -------------------------------------------------
	...
	echo
                ...
                mkdir  4_GWAS/
		mkdir  4_GWAS/${GWASRunName}_Analysis
	####	mkdir  4_GWAS/Phenotype
                mkdir  4_GWAS/Datasets
...
if [ "${GWASOverride,,}" == "t" ]; then
	...
	echo
		...
		VCF_Input=$"`find ${BaseName}_Imputation/${ImputationProject2Analyze}/ConcatImputation -maxdepth 1 -type f -name '1Imputed*'`"
		Sex_Input=$"`find ${BaseName}_Imputation/${ImputationProject2Analyze} -maxdepth 1 -type f -name 'Ody1_*QC2.fam'`"		
...
# GWAS Rscript:
${Rscript} $oddysseyPath/4_GWAS/.R_GWAS.R $Arg6 $Arg7 $Arg8 $Arg9 $Arg10 $Arg11 '$Arg12' $Arg13 $Arg14 $Arg15 $Arg16 $Arg17 $Arg18" > ${GWASSubDir}/1_${GWASRunName}_Analyze-Visualize.sh
