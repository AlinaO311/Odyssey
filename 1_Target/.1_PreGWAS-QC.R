 ...
 AnalyzeGeno <- function(WorkingDir, plink)  { 
	...
	# Get the Base Name of the Plink files currently housed in TargetData
	       qdir<-Sys.getenv("VARIABLENAME")
               pathname<-qdir[1]
	       system(paste0("ls ", WorkingDir ,"/*.log", "| awk -F/ '{print $NF}' | sed 's/.log*.//'"), intern=TRUE) ->BaseName
	...
	# Setup a QC Folder within the TargetData folder
		...
		system(paste0("mkdir -p", WorkingDir, "/Dataset_QC-Visualization"))
...
## Calculate the Varinat Missing Rate
	...
	system(paste0(plink, " --bfile", WorkingDir, BaseName, " --missing --out ",WorkingDir, "/Dataset_QC-Visualization/Missing"))
...
# Load the Missingness data
	lmiss <- fread(paste0(WorkingDir, "/Dataset_QC-Visualization/Missing.lmiss"), header = T)
...
## Output one JPEG Variant Call Rate File to the QC Folder
	...
	geom_histogram(aes(fill=..count..))	
		...
		jpeg(paste0(WorkingDir,"/Dataset_QC-Visualization/Missing.jpg"))
...
# Calculate Heterozygosity
	...
	cat('----------------------------\n\n')
	system(paste0(plink, " --bfile ", WorkingDir , BaseName, " --het --out ", WorkingDir, "/Dataset_QC-Visualization/Het"))

	cat('\n\n')

	het = fread(paste0(WorkingDir,"/Dataset_QC-Visualization/Het.het"), header = T)
...
## Load the .imiss file -- Missingness for individuals file	
	imiss = fread(paste0(WorkingDir, "/Dataset_QC-Visualization/Missing.imiss"), header = T)
...
## Plot the imiss vs. het Output to a jpeg file (Setup to show +/- X SD of Het Mean)
	jpeg(paste0(WorkingDir,"/Dataset_QC-Visualization/IndivMissingVSheterozygosity.jpg"))
...
## Export to 'fail-imisshet-qc.txt'
	write.table(failed.miss.het, file = paste0(WorkingDir, "/Dataset_QC-Visualization/failed.imisshet.txt"), col.names = T, row.names = F, quote = F, sep = "\t")
...
## Perform IBD Test to Assess Relatedness
	...
	system(paste0(plink, " --bfile ",WorkingDir, BaseName, " --mind 0.05 --geno 0.05 --make-bed --freq --out ", WorkingDir, "/Dataset_QC-Visualization/1_IBDDataset"))

	cat('\n\n')

## Prune the Dataset that has been corrected for poor SNP's and Individuals
	cat('\n\nPruning TEMP IBD Dataset\n-----------------------------------------\n\n')
	system(paste0(plink, " --bfile ", WorkingDir, "_Step0out/TargetData/Dataset_QC-Visualization/1_IBDDataset --indep-pairwise 50 5 0.2 --out ",WorkingDir, "/Dataset_QC-Visualization/2_IBDDataset_Pruned"))
...
## Import the '.genome file
	...
	Related <- fread(paste0(WorkingDir, '/Dataset_QC-Visualization/RelatedTest.genome'), h = T)
...
#Visualize Plotly IBD Plot
	...
	# Save Plotly-Plot to QC Folder
		# Plotely Dependencies will be plotted in the Directory where plot is saved
			htmlwidgets::saveWidget(IBD_Plot, WorkingDir, '/Dataset_QC-Visualization/IBD-Plot.html'), libdir = paste0(WorkingDir, '/Dataset_QC-Visualization/PlotlyDependencies/'), selfcontained=FALSE) 
...
##Output the 1st two Columns so we can remove those people from the GWAS
	...
	write.table(failed.related, file = paste0(WorkingDir, "/Dataset_QC-Visualization/failed.related.txt"), col.names = T, row.names = F, quote = F, sep = "\t")
...
## Load Related Check QC
	(failed.miss.het = read.table(file = paste0(WorkingDir, '/Dataset_QC-Visualization/failed.imisshet.txt'), header = T, colClasses = 'factor'))

	cat('List of the first individual in the pair who failed IBD\n')
	cat('Note: In any IBD pair only ONE individual will be dropped\n')
	cat('--------------------------------\n')

	failed.related = read.table(file = paste0(WorkingDir, "/Dataset_QC-Visualization/failed.related.txt"), header = T, colClasses = 'factor')
...
## Output to a Text File in the QC Folder
	write.table(failed.individualuniq, file = paste0(WorkingDir, "/Dataset_QC-Visualization/failed.qc.txt"), sep = "\t", col.names = F, row.names = F, quote = F)
...
## Cleanup the Junk
	...
	system(paste0('rm -f ', WorkingDir , '/Dataset_QC-Visualization/2*'))
	system(paste0('rm -f ', WorkingDir , '/Dataset_QC-Visualization/1*'))
...
sink(paste0(WorkingDir, '/Dataset_QC-Visualization/README_QC'))
