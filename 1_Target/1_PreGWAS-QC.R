...
# Read in the Command arguments into the R environment  
 ...
 AnalyzeGeno <- function(WorkingDir, plink)  { 
	...
	# Get the Base Name of the Plink files currently housed in TargetData
	       system(paste0("ls *.log", "| awk -F/ '{print $NF}' | sed 's/.log*.//'"), intern=TRUE) ->BaseName
	# Setup a QC Folder within the TargetData folder
		cat("\n\n===================================================\n")
		cat("Creating QC Folder within TargetData folder\n")
		cat("--------------------------------\n\n")
		system("mkdir Dataset_QC-Visualization")
# ============================
## Assess Variant Missingness
# ============================
## Calculate the Variant Missing Rate
	cat("\n\n===================================================\n")
   	cat("Assessing Missingness for Individuals & Variants\n")
  	cat("--------------------------------\n\n")
   	system(paste0(plink, " --bfile ", BaseName ," --missing --out Missing"))
        cat("--------------------------------\n\n")
        cat("--------------------------------\n\n") 
        system("mv Missing* Dataset_QC-Visualization")      
  	cat("\n\n")
...
# Load the Missingness data
	  lmiss <- fread("Dataset_QC-Visualization/Missing.lmiss", header = T)
	# ============================
...
## Output one JPEG Variant Call Rate File to the QC Folder
	  ...
	  jpeg("Dataset_QC-Visualization/Missing.jpg")
...
# Detect if running script interactively or non-interactively
	  ...
	  system(paste0(plink, " --bfile ",BaseName ," --het --out Het"), intern=TRUE)
	  system("mv Het* Dataset_QC-Visualization/", intern=TRUE)
	  
	  cat("\n\n")
	  
	het = fread("Dataset_QC-Visualization/Het.het", header = T)
...
## Load the .imiss file -- Missingness for individuals file	
	imiss = fread("Dataset_QC-Visualization/Missing.imiss", header = T)
...
# =========================================
	...
	jpeg("Dataset_QC-Visualization/IndivMissingVSheterozygosity.jpg")
	...
	write.table(failed.miss.het, file = "Dataset_QC-Visualization/failed.imisshet.txt", col.names = T, row.names = F, quote = F, sep = "\t")
	# ==============================
	...
	cat("--------------------------------\n\n")
	system(paste0(plink, " --bfile ",BaseName, " --mind 0.05 --geno 0.05 --make-bed --freq --out 1_IBDDataset"))
	cat("\n\n")
	## Prune the Dataset that has been corrected for poor SNPs and Individuals
	cat("\n\nPruning TEMP IBD Dataset\n-----------------------------------------\n\n")
	system(paste0(plink, " --bfile 1_IBDDataset --indep-pairwise 50 5 0.2 --out 2_IBDDataset_Pruned"))
	...
	system(paste0(plink, " --bfile 1_IBDDataset --extract 2_IBDDataset_Pruned.prune.in --mind 0.1 --genome --min 0.1875 --read-freq 1_IBDDataset.frq --out RelatedTest"))
	system(paste0("mv 1_IBDDataset* Dataset_QC-Visualization/"))
	system(paste0("mv 2_IBDDataset_Pruned* Dataset_QC-Visualization/"))
        system(paste0("mv RelatedTest* Dataset_QC-Visualization/"))
		## Import the ".genome file
	cat("\n\nOutputting Interactive Plotly IBD Plots to Folder\n")
	cat("--------------------------------------------------------\n\n")
	Related <- fread("Dataset_QC-Visualization/RelatedTest.genome", header = T)
...
# Detect if running script interactively or non-interactively
	...
	# Plotely Dependencies will be plotted in the Directory where plot is saved
        cat("--------------------------------\n\n")
	htmlwidgets::saveWidget(IBD_Plot, "Dataset_QC-Visualization/IBD-Plot.html", libdir ="PlotlyDependencies", selfcontained=FALSE) 
	...
	write.table(failed.related, file = "Dataset_QC-Visualization/failed.related.txt", col.names = T, row.names = F, quote = F, sep = "\t")
	...
	(failed.miss.het = read.table(file ="Dataset_QC-Visualization/failed.imisshet.txt", header = T, colClasses = "factor"))
	cat("List of the first individual in the pair who failed IBD\n")
	cat("Note: In any IBD pair only ONE individual will be dropped\n")
	cat("--------------------------------\n")
	failed.related = read.table(file = "Dataset_QC-Visualization/failed.related.txt", header = T, colClasses = "factor")
	colnames(failed.related) <- c("FID", "IID")
	print(failed.related)
## Add or remove which files you want to combine (for instance some SexProblems are not really problems)
...
## Output to a Text File in the QC Folder
	write.table(failed.individualuniq, file =  "Dataset_QC-Visualization/failed.qc.txt", sep = "\t", col.names = F, row.names = F, quote = F)
...
## Cleanup the Junk
	...
	system(paste0("rm -f Dataset_QC-Visualization/2*"), intern=TRUE)
	system(paste0("rm -f Dataset_QC-Visualization/1*"), intern=TRUE)
...
# Create a custom Readme file in the target directory to explain what is being populated
	...
	cat("--------------------------------\n\n")
sink("Dataset_QC-Visualization/README_QC")
...
1) Failed QC Lists: These lists contain the FID and IID of individuals who failed the user-specified cutoff criteria
	a) failed.imisshet.txt -- Contains list of IDs who failed the heterozygosity test and who contained a high degree of missing genotypes
